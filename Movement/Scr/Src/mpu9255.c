...
extern volatile uint8_t MPU_Rx_Flag;
...
//	printf("**************************** \r\n");
	...
	//HAL_I2C_Mem_Read_DMA(I2Cx, MPU9250_ADDRESS, WHO_AM_I_MPU9250, 1, &readData, 1);
//	while(MPU_Rx_Flag == 0){
//		vTaskDelay(500);
//	}
//	MPU_Rx_Flag = 0;
	...
	if (readData == 113) {
		...
		writeData = 0x01;
		HAL_I2C_Mem_Write(I2Cx, MPU9250_ADDRESS, INT_ENABLE, 1, &writeData, 1, i2c_timeout);
...
//	HAL_I2C_Mem_Read_DMA(I2Cx, MPU9250_ADDRESS, INT_STATUS, 1, &Data, 1);
//	while(MPU_Rx_Flag == 0){
//		vTaskDelay(50);
//	}
//	MPU_Rx_Flag = 0;
...
//  HAL_I2C_Mem_Read_DMA(I2Cx, MPU9250_ADDRESS, ACCEL_XOUT_H, 1, &rawAccelData[0], 6);
//  while(MPU_Rx_Flag == 0){
//	  vTaskDelay(50);
//  }
//  MPU_Rx_Flag = 0;
...
//  HAL_I2C_Mem_Read_DMA(I2Cx, MPU9250_ADDRESS, GYRO_XOUT_H, 1, &rawGyroData[0], 6);
//  while(MPU_Rx_Flag == 0){
//	vTaskDelay(50);
//  }
  MPU_Rx_Flag = 0;
...
//	HAL_I2C_Mem_Read_DMA(I2Cx, AK8963_ADDRESS, AK8963_ST1, 1, &readData, 1);
//	while(MPU_Rx_Flag == 0){
//		vTaskDelay(50);
//	}
	MPU_Rx_Flag = 0;
...
//		HAL_I2C_Mem_Read_DMA(I2Cx, AK8963_ADDRESS, AK8963_XOUT_L, 1, &rawMagData[0], 7);
//		while(MPU_Rx_Flag == 0){
//			vTaskDelay(50);
//		}
//		MPU_Rx_Flag = 0;
...
void calibrateMag(I2C_HandleTypeDef *I2Cx, float * dest1, float * dest2){
  ...
  osDelay(4000);
   ...
   for(ii = 0; ii < sample_count; ii++) {
    ...
    mag_scale[0]  = (mag_max[0] - mag_min[0])*magCalibration[0]/2;  // get average x axis max chord length in counts
    mag_scale[1]  = (mag_max[1] - mag_min[1])*magCalibration[1]/2;  // get average y axis max chord length in counts
    mag_scale[2]  = (mag_max[2] - mag_min[2])*magCalibration[2]/2;  // get average z axis max chord length in counts
...
//	readData = readData | Gscale << 3; // Set full scale range for the gyro
	readData |=  (3<<0)|(3<<3); // Set full scale range for the gyro
	osDelay(100);
	//printf("bal bla %x\r\n", readData);
	writeData = readData;
